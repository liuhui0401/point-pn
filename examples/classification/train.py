...
def main(gpu, cfg, profile=False):
    ...
    # # test nn
    # cache_keys = torch.load('/ssddata/pointpn_next/scan_f.pt')
    # cache_values = torch.load('/ssddata/pointpn_next/scan_t.pt')
    # cache_keys /= cache_keys.norm(dim=-1, keepdim=True)
    # cache_keys = cache_keys.permute(1, 0)
    # best_alpha, best_beta, best_oacc, best_macc = validate_nn(model, val_loader, cfg, cache_keys, cache_values)
    # if writer is not None:
    #     writer.add_scalar('test_oa', best_oacc, best_epoch)
    #     writer.add_scalar('test_macc', best_macc, best_epoch)
...
def train_one_epoch(model, train_loader, criterion, optimizer, scheduler, epoch, cfg):
    ...
    for idx, data in pbar:
        ...
        # all_target = torch.cat((all_target, target.cpu()), dim=0)
    ...
    # torch.save(all_logits, '/ssddata/pointpn_next/scan_f.pt')
    # all_target = F.one_hot(all_target.long())
    # torch.save(all_target, '/ssddata/pointpn_next/scan_t.pt')
...
@torch.no_grad()
def validate_nn(model, val_loader, cfg, cache_keys, cache_values):
    model.eval()
    npoints = cfg.num_points
    pbar = tqdm(enumerate(val_loader), total=val_loader.__len__())
    all_feature, all_target = torch.Tensor(), torch.Tensor()
    for idx, data in pbar:
        for key in data.keys():
            data[key] = data[key].cuda(non_blocking=True)
        target = data['y']
        ...
        data['x'] = points[:, :, :cfg.model.in_channels].transpose(1, 2).contiguous()
        feature = model(data)
        all_feature = torch.cat((all_feature, feature.cpu()), dim=0)
        all_target = torch.cat((all_target, target.cpu()), dim=0)
    all_feature /= all_feature.norm(dim=-1, keepdim=True)
    alpha_list = [i * (300.0 - 1.0) / 200 + 1 for i in range(200)]
    beta_list = [i * (3 - 0.01) / 20 + 0.01 for i in range(20)]
    affinity = all_feature @ cache_keys
    best_oacc, best_macc = 0., 0.
    best_alpha, best_beta = 0., 0.
    for alpha in alpha_list:
        for beta in beta_list:
            logits = ((-1) * (beta - beta * affinity)).exp() @ cache_values.float()
            preds = logits.max(dim=1)[1]
            oacc = metrics.accuracy_score(all_target, preds)
            macc = metrics.balanced_accuracy_score(all_target, preds)
            if oacc > best_oacc:
                best_oacc = oacc
                best_macc = macc
                best_alpha = alpha
                best_beta = beta
    return best_alpha, best_beta, best_oacc, best_macc