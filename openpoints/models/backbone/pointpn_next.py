...
from ..layers import CreateResConvBlock1D, CreateResConvBlock2D, create_convblock1d, create_convblock2d, create_act, CHANNEL_MAP, \
...
class SetAbstraction(nn.Module):
    ...
    def __init__(self,
        ...
        if self.is_head:
            ...
            create_conv = CreateResConvBlock2D
            convs.append(create_conv(channels,
                                        norm_args=norm_args,
                                        act_args=act_args,
                                        **conv_args))
        ...
        #                     )
        # else:
        #     create_conv = CreateResConvBlock1D
        #     convs.append(create_conv(channels,
        #                             norm_args=norm_args,
        #                             act_args=act_args,
        #                             **conv_args))
    ...
    def forward(self, px):
        ...
        if self.is_head:
            ...
            # x = self.convs(self.pool(xj))
            # x = self.pool(self.convs(torch.cat((dp, xj), dim=1)))
            # if self.use_res:
            #     x = self.act(x + identity)
            # x = self.pool(xj)
            # if self.use_res:
            #     x = self.convs(self.act(x+identity))
            p = new_p
        return p, x
