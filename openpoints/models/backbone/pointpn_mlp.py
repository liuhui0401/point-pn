    ...
    def forward(self, xyz, points):
        ...
        if self.normalize is not None:
            ...
            if self.normalize == "anchor":
                ...
                mean_xyz = new_xyz
                mean_xyz = mean_xyz.unsqueeze(dim=-2)
...
class PosPool_Block(nn.Module):
    def __init__(self, out_channels, pos_layers_num=1, res_expansion=1, activation="relu", last=False):
        """
        input: [b,g,k,d]: output:[b,d,g]
        :param channels:
        :param blocks:
        ...
        for i in range(pos_layers_num):
            conv_layer = PosExtraction(out_channels, 1, activation=activation, res_expansion=res_expansion)
...
class PointPNEncoder_mlp(nn.Module):
    def __init__(self, points=1024, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=False, use_xyz=False, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[24, 24, 24, 24], reducers=[2, 2, 2, 2], in_channels=3, **kwargs):
        super(PointPNEncoder_mlp, self).__init__()
        ...
        for i in range(len(pos_blocks)):
            ...
            pos_block_module = PosPool_Block(out_channel, pos_block_num, res_expansion, activation, last)
    ...
    def forward_cls_feat(self, p, x=None):
        ...
        for i in range(self.stages):
            ...
            xyz, re_xyz, x = self.local_grouper_list[i](xyz, x.permute(0, 2, 1))
            # pdb.set_trace()
            xyz, x = self.pos_blocks_list[i](xyz, re_xyz, x)
        x = F.adaptive_max_pool1d(x, 1).squeeze(dim=-1) + x.mean(-1)
        return x  # 32 288



def PointPN_mlp(num_classes=40, **kwargs) -> PointPNEncoder_mlp:
    return PointPNEncoder_mlp(points=1024, num_classes=num_classes, embed_dim=36, group=1, res_expansion=0.25,
                    activation="relu", bias=False, use_xyz=False, normalize="anchor",
